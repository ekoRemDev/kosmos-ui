  ...
  void paint(
    ...
    if (rangeValues != null) {
      TextSpan span = TextSpan(
        ...
        text: thumb == Thumb.start ? rangeValues!.start.toInt().toString() : rangeValues!.end.toInt().toString(),
      );
      ...
      thumb == Thumb.start
          ? tp.paint(canvas, Offset(center.dx - tp.width / 1.8, center.dy + thumbSize))
          : tp.paint(canvas, Offset(center.dx - tp.width / 1.9, center.dy + thumbSize));
    }
...
Path shape(double size, Offset thumbCenter, double radius, {bool invert = false}) {
  final Path thumbPath = Path();
  final double halfSize = size / 1.5;
  final double sign = invert ? -1.0 : 1.0;
  thumbPath.moveTo(thumbCenter.dx + halfSize * sign, thumbCenter.dy);
  thumbPath.addOval(Rect.fromCircle(center: Offset(thumbCenter.dx, thumbCenter.dy), radius: radius));
  thumbPath.close();
  return thumbPath;
}
