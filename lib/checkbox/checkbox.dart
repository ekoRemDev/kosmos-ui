...
class _Square extends StatelessWidget implements CustomCheckbox {
  ...
  Widget build(BuildContext context) {
    final themeData = loadThemeData(theme, themeName ?? "checkbox_square", () => const CustomCheckBoxThemeData())!;

    return GestureDetector(
      onTap: onTap,
      child: AnimatedContainer(
        duration: themeData.animationDuration ?? const Duration(milliseconds: 100),
        ...
        child: isChecked
            ? Icon(
                iconData ?? Icons.done,
                color: selectedIconColor ?? themeData.selectedIconColor ?? Colors.white,
                size: iconSize ?? 12,
              )
            : null,
      ),
...
class _Circle extends StatelessWidget implements CustomCheckbox {
  ...
  @override
  Widget build(BuildContext context) {
    final themeData = loadThemeData(theme, themeName ?? "checkbox_circle", () => const CustomCheckBoxThemeData())!;
      ...
      child: AnimatedContainer(
        ...
        child: isChecked
            ? Padding(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: selectedColor ?? themeData.selectedColor ?? const Color(0xFF02132B),
                    shape: BoxShape.circle,
                  ),
                ),
              )
